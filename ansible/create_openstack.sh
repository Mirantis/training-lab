...
if [ "$ACTION" == "delete" ]; then
  $DOCKER_RUN "ansible-playbook --private-key \$HOME/.ssh/id_rsa --extra-vars \"cloud_platform=$CLOUD_PLATFORM terraform_state=absent prefix=${USER}\" site.yml"
else
  $DOCKER_RUN "ansible-playbook --private-key \$HOME/.ssh/id_rsa --extra-vars \"cloud_platform=$CLOUD_PLATFORM terraform_state=present prefix=${USER}\" site.yml"
fi
